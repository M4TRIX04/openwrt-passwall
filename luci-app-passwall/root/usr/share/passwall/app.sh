...
run_socks() {
	...
	hysteria2)
		...
		[ -n "$no_run" ] || ln_run "$(first_type $(config_t_get global_app hysteria2_file))" "hysteria2" $log_file -c "$config_file" client
	;;
	tuic)
		json_add_string "local_addr" "$bind"
		json_add_string "local_port" "$socks_port"
...
run_redir() {
	...
	UDP)
		...
		hysteria2)
			json_add_string "local_udp_redir_port" "$local_port"
			lua $UTIL_HYSTERIA2 gen_config "$(json_dump)" > $config_file
			ln_run "$(first_type $(config_t_get global_app hysteria2_file))" "hysteria2" $log_file -c "$config_file" client
		;;
		tuic)
...
get_config() {
	...
	REDIRECT_LIST="socks ss ss-rust ssr sing-box xray trojan-plus naiveproxy hysteria hysteria2"
	TPROXY_LIST="socks ss ss-rust ssr sing-box xray trojan-plus hysteria hysteria2"
